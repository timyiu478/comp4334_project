...
def on_message(data):
    ...
    socketio.send(jsonify({'datetime':now,'from':current_user.username,'data': data}), broadcast=True, to=to_room)
    socketio.send(jsonify({'datetime':now,'from':current_user.username,'data': data}), broadcast=True, to=from_room)
